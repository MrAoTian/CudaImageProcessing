...
// #define HBUF             // Activate for save all results
#define SAVE_RESULT         // Activate for save final results

// #define HVAR_SAFE_FREE(a) if(a != nullptr) delete a; a = nullptr;
#define HARR_SAFE_FREE(a) if(a != nullptr) delete[] a; a = nullptr;
#define CUDA_SAFE_FREE(a) if(a != nullptr) cudaFree(a); a = nullptr;
#define SAFE_RESET(a) if (a != nullptr) a.reset();
...
typedef union
{
    struct
    {
        unsigned short bit0 : 6;
        unsigned short bit1 : 10;
    } bit01;
...
__host__ __device__ __inline__ int dealBorder(int i, int sz)
{
    if (i < 0)
        return -i;
    if (i >= sz)
...
/* Get Exp Up Exponent */
inline int iExp2Up(const int a)
{
    return static_cast<int>(powf(2, ceilf(log2f(static_cast<float>(a)))));
}
